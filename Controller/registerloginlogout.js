...
const register = async (req, res) => {
    try {
        const { first_name, middle_name, last_name, email, password, contact } = req.body;
        ...
        // Create the new user
        const UserRegister = await admin.create({ first_name, middle_name, last_name, email, password: hashing, contact });

        sendMail(email,"Welcome to our website","",registerEmail(first_name))

        // Create JWT token
        const token = jwt.sign({ id: UserRegister._id }, process.env.SecretKey, { expiresIn: "15000h" });
...
const login = async(req,res)=>{
    ...
    const token = jwt.sign({id:EmailExists._id},process.env.SecretKey,{expiresIn:"15000h"})
